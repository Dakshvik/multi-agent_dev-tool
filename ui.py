...
st.caption("Paste your code and get an AI-powered multi-agent review: security, performance, fixes, tests, and docs.")
...
if run:
    ...
    if risk >= 0.7:
        st.error(f"High Risk: {risk:.1f}. Human approval required.")
    elif risk >= 0.4:
        st.warning(f"Medium Risk: {risk:.1f}.")
    else:
        st.success(f"Low Risk: {risk:.1f}. Looks good.")
    ...
    c4.metric("Validation", str(validation_status).upper())
    ...
    st.divider()
    tabs = st.tabs(["PR Report", "Fixed Code", "Tests", "Docs", "Security", "Performance", "Raw"])
